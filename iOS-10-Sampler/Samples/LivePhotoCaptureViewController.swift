...
//            guard let newVideoOrientation = videoOrientationFor(deviceOrientation), deviceOrientation.isPortrait || deviceOrientation.isLandscape else {
//                return
//            }
//            
...
//            }
//            
//            if let videoDevice = LivePhotoCaptureViewController.deviceWithMediaType(AVMediaTypeVideo, preferringPosition: preferredPosition) {
//                do {
//                    let videoDeviceInput = try AVCaptureDeviceInput(device: videoDevice)
//                    
//                    self.session.beginConfiguration()
...
//         `isRecording` property is back to NO — which happens sometime after this method
//         returns.
//         
...
//    }
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        if context == &sessionRunningObserveContext {
            ...
            DispatchQueue.main.async { [unowned self] in
                // Only enable the ability to change camera if the device has more than one camera.
//                self.cameraButton.isEnabled = isSessionRunning && AVCaptureDevice.devices(withMediaType: AVMediaTypeVideo).count > 1
...
//                self.livePhotoModeButton.isHidden = !(isSessionRunning && isLivePhotoCaptureSupported)
    ...
    func sessionWasInterrupted(notification: NSNotification) {
         ...
         */
        if let reasonIntegerValue = (notification.userInfo?[AVCaptureSessionInterruptionReasonKey] as AnyObject).integerValue, let reason = AVCaptureSessionInterruptionReason(rawValue: reasonIntegerValue) {
    ...
    @IBAction private func capturePhoto(_ photoButton: UIButton) {
        ...
        sessionQueue.async {
            ...
            let photoCaptureDelegate = LivePhotoCaptureDelegate(with: photoSettings, willCapturePhotoAnimation: {
                DispatchQueue.main.async { [unowned self] in
